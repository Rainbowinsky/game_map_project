...
const environmentSchema = z
  .object({
    ...
    jwtIssuer: z.string().trim().min(1).max(200),
    jwtAudience: z.string().trim().min(1).max(200),
    jwtAccessTtlSeconds: z.number().int().min(60).max(3_600),
...
export function loadAppConfig(environment: EnvironmentInput = process.env): AppConfig {
  ...
  return environmentSchema.parse({
    ...
    jwtIssuer: environment.JWT_ISSUER ?? 'fantasy-map-api',
    jwtAudience: environment.JWT_AUDIENCE ?? 'fantasy-map-web',
    jwtAccessTtlSeconds: integerEnvironmentValue('900', 60, 3_600).parse(
